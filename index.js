...
const vcard = 'BEGIN:VCARD\n' // KASIH WM GUE
            ...
            + 'FN:PATRICK\n' // JANGAN NGEYEL AJG
            + 'ORG:OWNER BOTZ;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6282215065695:+62 822-1506-5695\n' 
            + 'END:VCARD' 
prefix = '#'
blocked = []   
limitawal = 50
memberlimit = 4
cr = '*TAMA 𝐁𝐎𝐓 𝐕𝐄𝐑𝐈𝐅𝐈𝐄𝐃*'

/*
]=====> SETTINGS <=====[
*/
const ownerNumber = ["6282373870884@s.whatsapp.net","6282373870884@s.whatsapp.net"]
const botName = 'TAMA BOT'
const ownerName = 'TAMA'
const BarBarKey = 'IDxO1TFYnKADlX4pxcHa' // APIKEY DOWNLOAD FILE
const VhtearKey = 'PUNYATAMA' // YANG DI YOUTUBE
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	...
	client.on('credentials-updated', () => {
		fs.writeFileSync('./TAMA.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'ingfokan cuyy...')
	})
	fs.existsSync('./TAMA.json') && client.loadAuthInfo('./TAMA.json')
...
*/
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Teksnya mana kak? Contoh : ${prefix}nulis1 TAMA baik hati')
				ramlan = body.slice(8)
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Teksnya mana kak? Contoh : ${prefix}nulis2 TAMA baik hati')
				laysha = body.slice(8)
				reply('「❗」WAIT BRO NGAK LAMA KOK :V')
				...
				client.sendMessage(from, ''+tes+'\n\n_By : ⸸TAMA⸸Panutanque._', text, { quoted: mek })
				await limitAdd(sender)
				break
// NINJALOGO				
				case 'ninjalogo':
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply('「❗」Contoh : ${prefix}ninjalogoTAMA & Gans')
				reply(ind.wait())
...
// PORNHUB
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply('「❗」Contoh : ${prefix}pornhub TAMA & Hub')
				reply(ind.wait())
...
//GEMBOK TEXT
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply('[❗] Contoh : ${prefix}gemboktext 11 01 2021 & TAMA dan Nadia')
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply('[❗] Contoh : ${prefix}glitchtext TAMA & Gans')
					reply(ind.wait())
				...
				if (!isQuotedSticker) return reply('Reply atau Tag sticker yang mau dijadiin gambar kak >_<')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'nih beb [(^.^)]'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
                ...
                var gh = body.slice(12)
					...
					client.sendMessage(from, buffer, image, {caption: 'Nih beb >_<', quoted: mek})
					await limitAdd(sender)
					break
                   case 'stalkig':
                   if (isBanned) return reply(ind.baned())
				...
				case 'welcome':
                 ...
                 case 'event':
                  if (isBanned) return reply(ind.baned())                 
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
					if (args.length < 1) return reply('Fitur Sudah Aktif! btw, lu sangean ajg')
					if (Number(args[0]) === 1) {
...
*/
                ...
                addKoinUser('6282215065695@s.whatsapp.net', fee)
...
*/				
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Tuh Nomor Ownerku yang gans >_<, Ehh Kalo suka kamu boleh gak?',MessageType.text, { quoted: mek} )
					break					
/*
]=====> MAKER MENU <=====[
*/
...
har = body.slice(12)
reply('「❗」HARTA TAHTA KAMU AKU YANG PUNYA (><)')
